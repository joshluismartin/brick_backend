class Api::V1::CalendarController < Api::V1::BaseController
  ...
  def events
    ...
    if result[:success]
      render_success("Calendar events retrieved successfully", { events: result[:events] })
    else
  ...
  def bulk_sync
    ...
    if sync_habits
      current_user.habits.where(calendar_event_id: nil).find_each do |habit|
    ...
    if sync_milestones
      current_user.milestones.where(calendar_event_id: nil).find_each do |milestone|
        # Use milestone's target_date or default to 2 weeks from now
